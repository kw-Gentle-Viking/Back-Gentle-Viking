...
class CostModelCfg:
    ...
    slip_bps_mkt: float = 4.0
    slip_bps_lmt: float = 1.0
...
class ExecutionModel:
    ...
    def simulate(
        ...
        if isinstance(mid, (pd.Timestamp, np.datetime64)) or (not np.isfinite(mid)) or (not (1e-6 < float(mid) < 1e6)):
        ...
        if order_type == OrderType.MARKET:
            ...
            fee = abs(qty) * px * (self.cost_cfg.taker_fee_bps / 10_000.0)
        ...
        if crossed and (np.random.rand() < self.cost_cfg.maker_fill_prob):
            ...
            fee = abs(qty) * px * (self.cost_cfg.maker_fee_bps / 10_000.0)
